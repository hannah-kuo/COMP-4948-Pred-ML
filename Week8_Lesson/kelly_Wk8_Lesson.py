...
#         lr=0.001, batch_size=100, optimizer=optim.RMSprop,
#         callbacks=[EpochScoring(scoring='accuracy',
#         name='hannah_train_acc', on_train=True)] )
#     # Pipeline execution
#     model = net.fit(X_train, y_train)
#     return model, net
#
...
#
# def drawAccuracyPlot(net):
#     plt.plot(net.history[:, 'hannah_train_acc'], color='blue', label='train')
#     plt.plot(net.history[:, 'valid_acc'], color='orange', label='val')
#     plt.legend()
#     plt.show()
#
...
#
# def evaluateModel(model, X_test, y_test):
#     print("hannah's network model for college admissions")
#     print(model)
#     y_pred = model.predict(X_test)
#     report = classification_report(y_test, y_pred)
#     print(report)
...
# model  = buildModel(X_train, y_train)
#
# print("hannah's flu diagnosis network best parameters:")
# print(model.best_params_)
#
# # Evaluate the model.
# evaluateModel(model.best_estimator_, X_test, y_test)
