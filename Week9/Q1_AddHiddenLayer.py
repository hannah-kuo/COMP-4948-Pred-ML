...
class MyNeuralNet(nn.Module):
    def __init__(self, num_units=10, nonlin=F.relu):
        ...
        self.dense2 = nn.Linear(num_units, 10)  # new hidden layer
        self.output = nn.Linear(10, 1)

    def forward(self, X, **kwargs):
        X = self.nonlin(self.dense0(X))
        X = F.relu(self.dense1(X))
        X = self.nonlin(self.dense2(X))  # pass through new hidden layer
        X = self.output(X)
        return X
...
df = pd.read_csv('C:/PredML/petrol_consumption.csv')
X = df.copy()
